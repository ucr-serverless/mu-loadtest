...
class HttpClient {
	...
	start() {
		...
		/**
		 * GWU:Custom.
		 * Let's users change rps of the client overtime based on a fixed interval
		 */
		if (this.params.updateRPSCallback && this.params.rpsInterval) {
			this.rpsIntervalTimer = new HighResolutionTimer(this.params.rpsInterval * 1000, () => {
				this.params.requestsPerSecond = this.params.updateRPSCallback()/this.params.concurrency;
				this.options.agent.maxSockets = 10 + this.params.requestsPerSecond;  
